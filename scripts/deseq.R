...
run.deseq <- function(args)
{
  ...
  {
    stop("\nUSAGE:\n\tRscript deseq.R in.file out.folder")
  }
  ...
  diff.exp.analysis.results <-
    diff.exp.analysis.results[order(diff.exp.analysis.results$padj),]
  ...
  plot(diff.exp.analysis.results$baseMean,
       pmin(-log10(diff.exp.analysis.results$pvalue),50),
       log="x",
  ...
  dev.off()
  ## end copy code
  diff.exp.analysis.results$padj <-
    p.adjust(diff.exp.analysis.results$pvalue, method="BH")
  res.frame <- as.data.frame(diff.exp.analysis.results)
  res.frame$genes <- genes
  write.table(res.frame,
              file=paste(out.file,".results.tsv", sep=''),
              quote=F, sep="\t", row.names=F)
}

# run diff exp analysis
if (length(commandArgs(trailingOnly=T))) {
  run.deseq(commandArgs(trailingOnly=T))
} else {
  run.deseq(c("~/Desktop/", "~/Desktop/test.out"))
}
